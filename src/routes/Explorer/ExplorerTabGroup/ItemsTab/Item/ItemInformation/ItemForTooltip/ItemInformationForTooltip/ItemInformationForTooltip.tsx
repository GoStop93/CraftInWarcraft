...
import { ItemForTooltip } from '../ItemForTooltip';
import {
  StyledItemName,
  StyledItemLevel,
  StyledItemMainText,
  ...
  StyledPlayableClass,
} from './ItemInformationForTooltip.styles';

export const ItemInformationForTooltip = ({ children, id, recipe }: IItemInformationProps) => {
  ...
  return (
    <>
      <StyledItemName color={itemQuality}>
        {recipe ? (
          <ToolTip items content={<ItemForTooltip id={id} />}>
            {itemName}
          </ToolTip>
        ) : (
          <>{itemName}</>
      ...
      {itemNameDescription ? (
        <StyledItemNameDescription r={itemNameDescriptionR} g={itemNameDescriptionG} b={itemNameDescriptionB}>
          {itemNameDescription}
        </StyledItemNameDescription>
      ) : null}
      {itemLevel ? <StyledItemLevel>{itemLevel}</StyledItemLevel> : null}
      {itemBinding ? <StyledItemMainText>{itemBinding}</StyledItemMainText> : null}
      {limitCategory ? <StyledItemMainText>{limitCategory}</StyledItemMainText> : null}
      {unique ? <StyledItemMainText>{unique}</StyledItemMainText> : null}
      {expiration ? <StyledItemMainText>{expiration}</StyledItemMainText> : null}
      {toy ? <StyledToy>{toy}</StyledToy> : null}
      {gemProperties ? <StyledItemMainText>{gemProperties}</StyledItemMainText> : null}
      {itemClass && itemSubclass && itemSubclass !== 'Non-equippable' && itemSubclass !== 'Quest' ? (
        <StyledItemClassAndSubclass>
          <StyledItemMainText>{itemSubclass}</StyledItemMainText>
          {itemClass !== 'Quest' && itemClass !== 'Miscellaneous' ? <StyledItemMainText>{itemClass}</StyledItemMainText> : null}
        </StyledItemClassAndSubclass>
      ) : null}
      {itemArmor ? <StyledItemMainText>{itemArmor}</StyledItemMainText> : null}
      ...
      {weaponDps ? <StyledItemMainText>{weaponDps}</StyledItemMainText> : null}
      ...
      {durability ? <StyledItemMainText>{durability}</StyledItemMainText> : null}
        ...
        <StyledPlayableClassesContainer>
          ...
          {playableClasses.map((playableClasse, index) => (
            <StyledPlayableClass playableClasse={playableClasse} key={playableClasse}>
              {playableClasse}
              {index !== playableClasses.length - 1 ? ',' : ''}
            </StyledPlayableClass>
          ))}
        </StyledPlayableClassesContainer>
      ) : null}
      ...
      {requiresLevel ? <StyledItemMainText>{requiresLevel}</StyledItemMainText> : null}
      {setItems.length > 0 ? (
        <StyledSetBlock>
          <StyledSetName>{setName}</StyledSetName>
          {setItems.map((item) => (
            <StyledSet key={item.id}>{item.name}</StyledSet>
          ))}
          <StyledSetBonusBlock>
      ...
      {itemDescription && itemType !== 'Recipe' ? <StyledDescription>"{itemDescription}"</StyledDescription> : null}
      {itemDescription && itemType === 'Recipe' ? <StyledDescription recipe>Use: {itemDescription}</StyledDescription> : null}
      {requiresSkill ? <StyledItemMainText>{requiresSkill}</StyledItemMainText> : null}
      {charges ? <StyledItemMainText>{charges}</StyledItemMainText> : null}
      {priseHeader ? (
        <StyledPrice>
          {priseHeader}{' '}
          {priseGold !== '0' ? (
            <>
              {priseGold}
              <StyledPriceIcon src={Gold} />
            </>
          ) : null}
          {priseSilver !== '0' ? (
            <>
              {priseSilver}
              <StyledPriceIcon src={Silver} />
            </>
          ) : null}
          {priseCopper !== '0' ? (
            <>
              {priseCopper}
              <StyledPriceIcon src={Copper} />
            </>
          ) : null}
        </StyledPrice>
      ) : null}
      ...
      {reagents ? (
        <StyledReagents>
          {reagents.map((reagent, index) => (
            <StyledItemMainText key={reagent.name}>
